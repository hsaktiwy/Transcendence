    ...
    + added rest_framwork
        {
            ...
            CSRF_TRUSTED_ORIGINS = ["http://10.11.7.13:3000"] 
        }
...
hints:
    [=] if you need to connect your frontend with the backend, and get the error 401 UnAuthorized or 403 Forbiden,
        cooled be that the server refuse the connection, that because in our back end setting.py there is protect that use to limite the hosts to connect, and trust only (http://10.11.7.13:3000, http://127.0.0.1:3000, http://10.11.7.13:5173, http://127.0.0.1:5173), you can  you host and pore in this arrays in setting.py and the problem will be fixed:
        CORS_ORIGIN_WHITELIST = (
            "http://10.11.7.13:3000",
        ...
        CSRF_TRUSTED_ORIGINS = ["http://10.11.7.13:3000", "http://10.11.7.13:5173"]
...
guidness : 
    ...
    [+] to make our application accessable from the local network:
            ...
            CSRF_TRUSTED_ORIGINS = ["http://10.11.7.13:3000", "http://10.11.7.13:5173", "http://127.0.0.1:6379", "http://{YOUR_COMPUTER_IP_IN_THE_NETWORK}:5173"]
    ...
    [+] inplementig the celery for task sheceduling: base on ('https://www.caktusgroup.com/blog/2021/08/11/using-celery-scheduling-tasks/')
        ...
        + install && conifguration
            ...
            2 - In your Django settings file, add:
                ```
                    CELERY_BROKER_URL = "redis://10.11.7.13:6379/0"
                    CELERY_RESULT_BACKEND = "redis://10.11.7.13:6379/1"
                ```
